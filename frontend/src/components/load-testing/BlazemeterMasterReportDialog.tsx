...
function fmtPct(rate: number | null | undefined): string {
  ...
  // BlazeMeter returns either a fraction (0.0–1.0) or a percent (0–100).
...
export function BlazemeterMasterReportDialog({ masterId, testName, onClose }: Props) {
  ...
  const open = masterId !== null
  const title = testName || master?.name || (masterId ? `Master ${masterId}` : "Run report")
      ...
      <DialogContent className="max-w-6xl max-h-[92vh] overflow-y-auto">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                {master?.id != null && (
                  <span>
                    Master <code className="rounded bg-muted px-1 py-0.5">{master.id}</code>
                  </span>
                )}
                {master?.ended != null && (
        ...
        {report && !loading && (
          <div className="space-y-6">
            ...
            {/* CI status / thresholds */}
            {(ciPassed !== null && ciPassed !== undefined) || thresholds.length > 0 || ciFailures.length > 0 ? (
              <section>
                <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  Pass / fail gates
                </h3>
                    ...
                    <TableBody>
                      {labelRows.map((r, i) => (
                        <TableRow key={r.labelId ?? i}>
                          <TableCell className="max-w-[320px] truncate text-xs" title={r.labelName ?? ""}>
                            {r.labelName ?? "—"}
                          </TableCell>
                          ...
                          <TableCell className="text-right text-xs">{fmtPct(r.errorRate)}</TableCell>
                          <TableCell className="text-right text-xs">{fmtMs(r.avgResponseTime)}</TableCell>
                          ...
                          <TableCell className="text-right text-xs">{fmtMs(r.maxResponseTime)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
            ...
            {/* Sub-section fetch errors (graceful degradation feedback) */}
