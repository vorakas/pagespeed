...
// Match Obsidian-style wikilinks. The orchestrator sometimes leaks the
// table-cell form `[[Stem\|ID]]` (with backslash-escaped pipe) into the
// status-page headline paragraph, which is prose. Render the display
// portion as a styled chip so the dashboard never shows raw `[[…]]`
// markup. Works on historical payloads too — the cleanup happens at
// render time, independent of what the parser stored.
const HEADLINE_WIKILINK_RE = /\[\[([^\]]+?)\]\]/g

function renderHeadlineSegments(text: string): React.ReactNode[] {
