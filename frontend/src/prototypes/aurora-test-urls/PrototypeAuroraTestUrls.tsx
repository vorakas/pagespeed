...
} from "@/components/test-urls/TestProgressPanel"
...
/**
 * Aurora-register port of the Test URLs page. Reuses every production
 * component (TestResultsTable, TestProgressPanel, TestDetailDialog,
 * ScoreBadge) — the legacy `aurora-*` token re-map in `aurora.css` skins
 * them to the new register. The desktop/mobile toggle moves from the
 * page header (BeaconHeader doesn't accept actions) into the page body
 * row alongside Test All / Refresh.
 */
export function PrototypeAuroraTestUrls() {
  ...
  const loadResults = useCallback(
    async (siteId: number, selectedStrategy: Strategy) => {
  ...
  const handleStrategyChange = (values: string[]) => {
    ...
    if (value === "desktop" || value === "mobile") setStrategy(value)
  }

  const handleSiteChange = (value: string) => {
    const siteId = parseInt(value)
    if (!isNaN(siteId)) setActiveSiteId(siteId)
  }
  ...
  const handleTestAll = useCallback(async () => {
    ...
    for (let i = 0; i < allUrls.length; i++) {
      ...
      setTestProgress((prev) => ({ ...prev, currentUrl: urlData.url }))
      ...
      setTestProgress((prev) => ({ ...prev, completed: i + 1, successful, failed }))
    }

    setTestProgress((prev) => ({ ...prev, currentUrl: null, finished: true }))
    if (activeSiteId !== null) loadResults(activeSiteId, strategy)
    setTimeout(() => setTesting(false), 5000)
  }, [sites, strategy, activeSiteId, loadResults])

  const handleRetestUrl = useCallback(
    async (urlId: number, url: string) => {
      setRetestingUrlId(urlId)
      try {
        await api.testUrl(urlId, url, strategy)
        if (activeSiteId !== null) await loadResults(activeSiteId, strategy)
      } catch {
        // silent — UI shows spinner stop
      } finally {
        ...
        <div className="space-y-6 p-6">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <ToggleGroup
              ...
              className="bg-muted rounded-lg p-0.5 ml-auto"
            >
              <ToggleGroupItem
                value="desktop"
                aria-label="Desktop"
          ...
          {sites.length === 0 ? (
            <div className="aurora-panel overflow-hidden">
              <EmptyState
                ...
                actionHref="/prototype/setup/aurora"
              />
            </div>
          ) : (
            <Tabs
